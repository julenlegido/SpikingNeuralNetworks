...
def train_ann_dvs(
    ...
    model = ANN_Event_CNN(input_channels=2, input_size=32).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        acc = evaluate_ann_dvs(model, test_loader, device)
        test_accuracies.append(acc)

        print(f"Epoch {epoch+1}, Loss: {total_loss:.4f}")
        print(f"Test Accuracy: {acc:.2f}%")

    torch.save(model.state_dict(), "results/checkpoints/ann_dvs_20epochs.pth")
    ...
    with open("results/logs/ann_dvs_results_20epochs.json", "w") as f:
        json.dump(results, f)

    return model


def evaluate_ann_dvs(model, data_loader, device):
    model.eval()
    correct = 0
    total = 0
