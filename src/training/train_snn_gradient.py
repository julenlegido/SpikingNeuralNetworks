...
def train_snn_gradient(num_epochs=5, num_steps=25, batch_size=64, lr=1e-3, spike_prob_scale=0.5):
    ...
    for epoch in range(num_epochs):
        ...
        test_accuracy = evaluate_snn_gradient(
            model, test_loader, device, num_steps, spike_prob_scale
        )
...
def evaluate_snn_gradient(model, data_loader, device, num_steps, spike_prob_scale):
    model.eval()
    correct = 0
    total = 0
